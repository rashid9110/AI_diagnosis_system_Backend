...
class AI_Model_repo {
  ...
  async getPatientsWithImages(userId) {
    return await AI_Model_Schema.aggregate([
      {
        $match: {
  ...
  async getDiagnosisById(id) {
    return await AI_Model_Schema.findById(id).populate("userId", "name email");
  }

  // ✅ Get diagnoses by user
  async getDiagnosesByUser(userId) {
    return await AI_Model_Schema.find({ userId }).sort({ createdAt: -1 });
  }

  // ✅ Update diagnosis
  async updateDiagnosis(id, updateData) {
    return await AI_Model_Schema.findByIdAndUpdate(id, updateData, {
      new: true,
    });
  }
